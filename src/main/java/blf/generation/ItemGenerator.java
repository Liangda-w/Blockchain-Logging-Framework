...
public class ItemGenerator extends BaseGenerator {
    ...
    @Override
    public void enterLogEntryFilter(final BcqlParser.LogEntryFilterContext ctx) {
    ...
    @Override
    public void exitScope(final BcqlParser.ScopeContext ctx) {
    ...
    @Override
    public void exitExpressionStatement(final BcqlParser.ExpressionStatementContext ctx) {
        if (this.logEntry == null) {
            return;
        }
    ...
    private BcqlParser.MethodInvocationContext getMethodInvocationContext(final BcqlParser.ExpressionStatementContext ctx) {
    ...
    private String getTargetVariable(final BcqlParser.ExpressionStatementContext ctx) {
    ...
    private boolean containsFunction(final BcqlParser.ExpressionStatementContext ctx, final String methodName) {
        final BcqlParser.MethodInvocationContext mCtx = getMethodInvocationContext(ctx);
        return mCtx != null && containsFunction(mCtx, methodName);
    }

    private boolean containsFunction(final BcqlParser.MethodInvocationContext ctx, final String methodName) {
        return ctx.methodName.getText().equals(methodName);
    }

    // #region ValueDictionaryItem collection

    private void createValueDictionary(final String targetVariable, final BcqlParser.MethodInvocationContext ctx) {
    ...
    private String getValueMapDictVariable(BcqlParser.MethodInvocationContext ctx) {
        if (ctx.valueExpression().size() <= VALDICT_VAR_INDEX) {
            return null;
        }

        BcqlParser.ValueExpressionContext val = ctx.valueExpression(VALDICT_VAR_INDEX);
    ...
    private String getValueMapDictType(BcqlParser.MethodInvocationContext ctx) {
        if (ctx.valueExpression().size() <= VALDICT_VAR_INDEX) {
            return null;
        }
    ...
    private String getValueDictTargetType(BcqlParser.MethodInvocationContext ctx) {
        if (ctx.valueExpression().size() <= VALDICT_DEFAULTVAL_INDEX) {
            return null;
        }

        final BcqlParser.ValueExpressionContext valueCtx = ctx.valueExpression(VALDICT_DEFAULTVAL_INDEX);
        return valueCtx.literal() == null ? null : InterpreterUtils.literalType(valueCtx.literal());
    }

    private Object getValueDictDefaultValue(BcqlParser.MethodInvocationContext ctx) {
    ...
    public void createBitMapping(final String targetVariable, final BcqlParser.MethodInvocationContext ctx) {
    ...
    private String getBitMappingVariable(final BcqlParser.MethodInvocationContext ctx) {
        final BcqlParser.VariableNameContext varCtx = ctx.valueExpression().get(BITMAP_VAR_INDEX).variableName();
        return varCtx == null ? null : varCtx.getText();
    }

    private BigInteger getBitMappingFrom(final BcqlParser.MethodInvocationContext ctx) {
        return this.getBitMappingPosition(ctx, BITMAP_FROM_INDEX);
    }

    private BigInteger getBitMappingTo(final BcqlParser.MethodInvocationContext ctx) {
        return this.getBitMappingPosition(ctx, BITMAP_TO_INDEX);
    }

    private BigInteger getBitMappingPosition(final BcqlParser.MethodInvocationContext ctx, final int index) {
        final BcqlParser.ValueExpressionContext posCtx = ctx.valueExpression().get(index);
    ...
    private List<?> getLiteralValues(final BcqlParser.MethodInvocationContext ctx, int index, boolean arrayOnly) {
        if (ctx.valueExpression().size() <= index) {
            return null;
        }

        final BcqlParser.ValueExpressionContext valuesCtx = ctx.valueExpression(index);
        if (valuesCtx.literal() == null) {
            return null;
        }

        if (valuesCtx.literal().arrayLiteral() != null) {
            final BcqlParser.ArrayLiteralContext literalCtx = valuesCtx.literal().arrayLiteral();
            ...
            final BcqlParser.LiteralContext literalCtx = valuesCtx.literal();
            final String literal = literalCtx.getText();
